...
import com.lichkin.framework.db.beans.SysToiletR;
...
public class S extends LKApiBusGetListService<I, O, SysToiletEntity> {
	...
	protected void initSQL(I cin, ApiKeyValues<I> params, QuerySQL sql) {
		sql.select(SysToiletR.id);
		sql.select(SysToiletR.photo);

		int i = 0;
		LKDictUtils4Temporary.locationCode(sql, params.getCompId(), SysToiletR.locationCode, i++);

		sql.eq(SysToiletR.compId, params.getCompId());
	}

...
import com.lichkin.framework.db.beans.SysSupermarketR;
...
public class S extends LKApiBusGetListService<I, O, SysSupermarketEntity> {
	...
	protected void initSQL(I cin, ApiKeyValues<I> params, QuerySQL sql) {
		sql.select(SysSupermarketR.id);
		sql.select(SysSupermarketR.photo);

		int i = 0;
		LKDictUtils4Temporary.locationCode(sql, params.getCompId(), SysSupermarketR.locationCode, i++);

		sql.eq(SysSupermarketR.compId, params.getCompId());
	}
